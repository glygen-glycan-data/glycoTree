...
function bail($errCode, $dStr, $format, $data) {
	...
	unset($data['nodes']);
	unset($data['links']);
	unset($data['dp_distribution']);		
	unset($data['path_count']);
	...
	if ($clean === true) {
		...
		if ($sizeDiff == 0 ) {
			...
			if (strpos($logic, "abiotic")) { 
				...
				$sql = "SELECT enzymes.gene_name,enzymes.uniprot,enzymes.type,enzymes.species,enzyme_mappings.curated FROM enzyme_mappings,enzymes WHERE enzymes.enzyme_id=enzyme_mappings.enzyme_id AND enzyme_mappings.residue_id=? ORDER BY species"; // @@!!
				$stmt = $connection->prepare($sql);
				$stmt->bind_param("s", $ra);
				$stmt->execute(); 
				$result = $stmt->get_result();
				while ($row = $result->fetch_assoc()) {
					$row['curated'] = ("true" if ($row['curated'] == 1) else "false");
					$enzymes[] = $row;
				}
			}
		}
