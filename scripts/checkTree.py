...
for level,residues in leveltoresid.items():
    ...
    allrules = set((r[0],r[1],residuemap[r[2]],r[3]) for s in residues for r in gt.get_rules(s))
    good = [ True ] * len(residues)
    for i,s in enumerate(residues):
        if s and set(t[0] for t in gt.get_enzymes(s)) != allenz:
            good[i] = False
            continue
        if s and set((r[0],r[1],residuemap[r[2]],r[3]) for r in gt.get_rules(s)) != allrules:
            good[i] = False
            continue
    # print(residues)
    # print(good)
    ...
    edge = gt.get_toedge(list(filter(None,residues))[0])
    line = [ "%- 25s"%(level,), "%- 15s"%("%s %s %s"%edge,) if edge else " "*15 ] + [ "%- 10s"%(s,) for s in seedsanot ]
